"""
LTX-2 Image-to-Video Processor

Uses Gemma-3 text encoder and LTX-2 19B model architecture.
Handles start image input for I2V workflows.
"""
...
from utils.comfyui_workflow_utils import (
    inject_prompt_and_image_into_ltx2_video_workflow,
    materialize_start_image,
)


class LTX2ImageToVideoJobProcessor(ComfyUIProcessor, VideoOutputHandler):
    """Processor for LTX-2 image-to-video generation."""
    ...
    def process(self):
        ...
        if not self.job:
            self._fail_job(f"Job object is None for LTX2ImageToVideoJobProcessor. Cannot proceed.")
            return
        ...
        img_compression = inputs.get("img_compression")
        image_noise_scale = inputs.get("image_noise_scale")
        ...
        wf_ready = inject_prompt_and_image_into_ltx2_video_workflow(
            ...
            img_compression=img_compression, image_noise_scale=image_noise_scale
        )
