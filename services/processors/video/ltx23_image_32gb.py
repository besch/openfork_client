"""
LTX-2.3 Image-to-Video Processor (32GB VRAM tier)

Generates synchronized audio+video conditioned on a start image.
32GB tier runs at 1024x576 @ 153 frames (~6.4s).
Uses dev model + distilled LoRA for quality image-conditioned generation.
...
class LTX23ImageToVideo32GBJobProcessor(ComfyUIProcessor, VideoOutputHandler):
    """Processor for LTX-2.3 image-to-video + audio generation (32GB tier)."""
    ...
    def process(self):
        ...
        if not self.job:
            self._fail_job(f"Job object is None for LTX23ImageToVideo32GBJobProcessor. Cannot proceed.")
            return
        ...
        wf_ready = inject_prompt_and_image_into_ltx23_video_workflow(
            ...
            aspect_ratio, steps=steps, cfg_scale=cfg_scale, strength=strength, tier="32gb"
        )
