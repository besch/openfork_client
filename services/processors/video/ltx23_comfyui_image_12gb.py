"""LTX-2.3 Image-to-Video Processor (ComfyUI backend, 12GB VRAM)."""
...
class LTX23ComfyUIImageToVideoProcessor12GB(ComfyUIProcessor, VideoOutputHandler):
    """ComfyUI-based LTX-2.3 I2V processor, 12GB tier (544x304, 81 frames)."""
    ...
    def process(self):
        ...
        if not self.job:
            self._fail_job(f"Job is None for LTX23ComfyUIImageToVideoProcessor12GB.")
            return
        ...
        logging.info(
            f"LTX-2.3 ComfyUI I2V 12GB job {self.job_id} completed: {video_storage_path}"
        )
