"""
SVI 2.0 Film Image-to-Video Processor

Generates multi-scene videos with seamless scene transitions.
Uses WAN 2.2 base model + SVI LoRA with 5 motion frames for clip coherence.

Key characteristics:
- Uses 5 motion frames (last 5 frames) for cross-clip coherence
- Zero padding (ref_pad_num=0) for scene flexibility
- Different seeds per clip for variety
- Designed for short films, scene transitions, storytelling
"""
...
class SVIFilmImageToVideoJobProcessor(ComfyUIProcessor, VideoOutputHandler):
    """Processor for SVI 2.0 Film image-to-video generation.
    
    SVI-Film is designed for multi-scene videos where subjects can move
    in and out of frame. It uses 5 motion frames for better cross-clip
    coherence and zero padding for scene transition flexibility.
    """
    ...
    def process(self):
        ...
        if not self.job:
            self._fail_job(f"Job object is None for SVIFilmImageToVideoJobProcessor. Cannot proceed.")
            return
        ...
        num_clips = inputs.get("num_clips", 1)
        num_motion_frames = inputs.get("num_motion_frames", 5)  # SVI-Film uses 5 motion frames

        wf_ready = self._inject_svi_film_params(
            workflow_data, 
        ...
        logging.info(f"SVI-Film job {self.job_id} completed successfully")

    def _inject_svi_film_params(
        self,
        ...
        """Inject SVI-Film specific parameters into workflow.
        
        SVI-Film uses:
        - 5 motion frames (last 5 frames) for cross-clip coherence
        - Zero padding (ref_pad_num=0) for scene flexibility
        ...
        for node in api_graph.values():
            ...
            if class_type == "CLIPTextEncode":
                ...
                # Set motion frames for SVI-Film mode
                if "num_motion_frames" in node["inputs"]:
                    node["inputs"]["num_motion_frames"] = num_motion_frames
