"""
LTX-2.3 Text-to-Video Processor (32GB VRAM tier)

Generates synchronized audio+video from a text prompt.
32GB tier runs at 1024x576 @ 153 frames (~6.4s) vs 768x432 @ 121 frames on 24GB.
...
class LTX23TextToVideo32GBJobProcessor(ComfyUIProcessor, VideoOutputHandler):
    """Processor for LTX-2.3 text-to-video + audio generation (32GB tier)."""
    ...
    def process(self):
        ...
        wf_ready = inject_prompt_into_ltx23_video_workflow(
            ...
            steps=steps, cfg_scale=cfg_scale, tier="32gb"
        )
