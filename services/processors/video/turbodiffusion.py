    ...
    def _download_output(self, remote_job_id: str) -> Optional[str]:
        ...
        try:
            ...
            return recover_output_from_clean_container_exit(
                self,
                local_path,
                container_output_path=f"/opt/TurboDiffusion/outputs/{remote_job_id}/generated_video.mp4",
                extensions=(".mp4",),
...
class TurboDiffusionT2VJobProcessor(TurboDiffusionBaseProcessor):
    ...
    def process(self):
        ...
        # TurboDiffusion only supports 1, 2, 3, or 4 steps
        num_steps = max(1, min(4, int(num_steps)))
