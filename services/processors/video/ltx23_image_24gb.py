"""
LTX-2.3 Image-to-Video Processor (24GB VRAM tier)
...
class LTX23ImageToVideo24GBJobProcessor(ComfyUIProcessor, VideoOutputHandler):
    """Processor for LTX-2.3 image-to-video + audio generation (24GB tier)."""
    ...
    def process(self):
        ...
        if not self.job:
            self._fail_job(f"Job object is None for LTX23ImageToVideo24GBJobProcessor. Cannot proceed.")
            return
        ...
        wf_ready = inject_prompt_and_image_into_ltx23_video_workflow(
            ...
            aspect_ratio, steps=steps, cfg_scale=cfg_scale, strength=strength, tier="24gb"
        )
