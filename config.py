...
# User-facing storage budget for OpenFork Docker images.
# The desktop app exposes this as a GB slider in Docker Management settings.
# The download manager treats this as the primary cache contract: when cached
...
# These trigger LRU eviction independent of the user cache budget.
#   Healthy:  free > DISK_PRESSURE_HEALTHY_GB         → honor the user cache budget.
#   Pressure: CRITICAL < free <= HEALTHY              → enforce budget; idle × 0.5.
...
def _apply_overrides() -> None:
    ...
    _dict_keys = ["POLICY_IDLE_TIMEOUT_MINUTES"]
