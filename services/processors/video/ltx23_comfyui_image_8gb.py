"""LTX-2.3 Image-to-Video Processor (ComfyUI backend, 8GB VRAM)."""
...
class LTX23ComfyUIImageToVideoProcessor8GB(ComfyUIProcessor, VideoOutputHandler):
    ...
    def process(self):
        ...
        if not self.job:
            self._fail_job(f"Job is None for LTX23ComfyUIImageToVideoProcessor8GB.")
            return
        ...
        logging.info(
            f"LTX-2.3 ComfyUI I2V 8GB job {self.job_id} completed: {video_storage_path}"
        )
