...
if [ "$RUN_DGN_CLIENT" = "true" ]; then
  ...
  docker run --rm \
    ...
    -v /data:/data \
    "$DOCKER_TAG" \
    bash -c 'set -euo pipefail; raw_base="https://raw.githubusercontent.com/besch/openfork_client/${OPENFORK_CLIENT_SCRIPT_REF:-main}"; curl --fail --location --proto "=https" --tlsv1.2 --silent --show-error "$raw_base/start_cloud.sh" -o /tmp/start_cloud.sh; bash /tmp/start_cloud.sh'
else
