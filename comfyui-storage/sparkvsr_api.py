...
_INFERENCE_WRAPPER.write_text(
    ...
    "_orig_text = T5EncoderModel.forward\n"
    "_enc_done = False\n"
    "_fwd_done = False\n"
    # Phase 1 — VAE encode: offload transformer+text_encoder to CPU, don't restore
    ...
    # Phase 2 — text encoding: temporarily GPU, back to CPU after each call
    "def _text_fwd(self, *a, **kw):\n"
    "    was_cpu = False\n"
    "    try:\n"
    "        if next(self.parameters()).device.type == 'cpu':\n"
    "            self.to('cuda'); was_cpu = True\n"
    "    except StopIteration: pass\n"
    "    try:\n"
    "        return _orig_text(self, *a, **kw)\n"
    "    finally:\n"
    "        if was_cpu:\n"
    "            self.to('cpu'); torch.cuda.empty_cache()\n"
    "            print('[sparkvsr-patch] text_encoder→CPU after encoding', flush=True)\n"
    # Phase 3 — transformer denoising: lazily restore transformer on first call
    # text_encoder stays on CPU, freeing ~6 GB for activation memory
    ...
    "T5EncoderModel.forward = _text_fwd\n"
