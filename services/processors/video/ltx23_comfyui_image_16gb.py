"""LTX-2.3 Image-to-Video Processor (ComfyUI backend, 16GB VRAM)."""
...
class LTX23ComfyUIImageToVideoProcessor16GB(ComfyUIProcessor, VideoOutputHandler):
    """ComfyUI-based LTX-2.3 I2V processor, 16GB tier (576x320, 97 frames)."""
    ...
    def process(self):
        ...
        if not self.job:
            self._fail_job(f"Job is None for LTX23ComfyUIImageToVideoProcessor16GB.")
            return
        ...
        logging.info(
            f"LTX-2.3 ComfyUI I2V 16GB job {self.job_id} completed: {video_storage_path}"
        )
