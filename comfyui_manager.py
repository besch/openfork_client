...
def get_workflow_output(prompt_id, terminal_node_ids=None, timeout_sec=7200):
    ...
    try:
        ws.connect(COMFYUI_URL.format(client_id), timeout=600)
        logging.info(f"Successfully connected to ComfyUI WebSocket at {COMFYUI_URL.format(client_id)}")
    except Exception as e:
        logging.error(f"Failed to connect to ComfyUI WebSocket at {COMFYUI_URL.format(client_id)}: {e}")
    ...
    try:
        while True:
            if (time.time() - start_ts) > timeout_sec:
                ...
                history_outputs = fetch_history_outputs(prompt_id)
                if history_outputs:
...
def fetch_history_outputs(prompt_id):
    ...
    try:
        history_url = f"{http_base}/history?prompt_id={prompt_id}"
