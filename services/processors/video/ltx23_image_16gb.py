"""
LTX-2.3 Image-to-Video Processor (16GB VRAM tier)

Same audio-video generation as the 24GB tier, but uses lower resolution
(512x288 @ 97 frames) to fit within 16GB VRAM activation budget.
Requires 64GB+ system RAM for CPU offloading the 46GB model.
"""
...
class LTX23ImageToVideo16GBJobProcessor(ComfyUIProcessor, VideoOutputHandler):
    """Processor for LTX-2.3 image-to-video + audio generation (16GB tier)."""
    ...
    def process(self):
        ...
        wf_ready = inject_prompt_and_image_into_ltx23_video_workflow(
            ...
            aspect_ratio, steps=steps, cfg_scale=cfg_scale, strength=strength, tier="16gb"
        )
